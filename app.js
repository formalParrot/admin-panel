...
router.post("/login", loginLimiter, async (req, res) => {
	...
	const valid = await bcrypt.compare(pin, process.env.ADMIN_PIN);
...
router.post("/lock", requireAdmin, async (req, res) => {
	...
	const base = (
		process.env.LOCK_INTERNAL_URL ||
		process.env.LOCK_API_URL ||
		""
	).replace(/\/+$/, "");
	const url = `${base}${path}`;
	...
	if (response.status === 502) {
		console.error(
			`[lock] ${action} got 502 from upstream (origin behind Cloudflare likely down):`,
		);
		console.error(`[lock] body (first 400 chars):`, text.slice(0, 400));
	} else {
		console.log(
			`[lock] ${action} response (first 500 chars):`,
			text.slice(0, 500),
		);
	}
...
router.all("/f42/*splat", requireAdmin, async (req, res) => {
	...
	try {
		const response = await fetch(
			`${(process.env.F42_INTERNAL_URL || "https://api.justparrot.me").replace(/\/+$/, "")}/f42/${splat}`,
			{
				method: req.method,
				headers: {
					"Content-Type": "application/json",
...
server.on("upgrade", (req, socket, head) => {
	...
	console.log(
		`[f42:ws] session id after unsign: ${sid ? "(valid)" : "(invalid/none)"}`,
	);
	...
	const isAuthorized = () => {
		...
		if (
			adminKey &&
			providedKey &&
			Buffer.from(providedKey).length === Buffer.from(adminKey).length
		) {
			try {
	...
	const proceed = (authorized) => {
		...
		wss.handleUpgrade(req, socket, head, (clientWs) => {
			...
			const wsBase =
				process.env.F42_WS_INTERNAL_URL || "wss://api.justparrot.me";
			const upstreamUrl = `${wsBase}/f42/${splat}${qs ? `?${qs}` : ""}`;

			console.log(
				`[f42:ws] client upgraded, connecting upstream: ${upstreamUrl}`,
			);
			...
			clientWs.on("close", () => {
				console.log(`[f42:ws] client closed`);
				upstream.close();
			});
	...
	sessionStore.get(sid, (err, session) => {
		console.log(
			`[f42:ws] session lookup err=${!!err} admin=${session?.admin === true}`,
		);
